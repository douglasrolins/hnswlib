 ...
 * @param <TDistance> Type of distance between items (expect any numeric type:
 *                    float, double, int, ..)
 * @see <a href="https://arxiv.org/abs/1603.09320"> Efficient and robust
		...
		implements Index<TId, TVector, TItem, TDistance>
{
					...
					synchronized (newNode)
					{
						...
						if (currObj != null)
						{
							...
							if (newNode.maxLevel() < entryPointCopy.maxLevel())
							{
								...
								for (int activeLevel = entryPointCopy.maxLevel(); activeLevel > newNode
									...
									while (changed)
									{
										...
										synchronized (currObj)
										{
											...
											for (int i = 0; i < candidateConnections.size(); i++)
											{
												...
												TDistance candidateDistance = distanceFunction.distance(item.vector(),
							...
							for (int level = Math.min(randomLevel, entryPointCopy.maxLevel()); level >= 0; level--)
							{
								...
								{
									TDistance distance = distanceFunction.distance(item.vector(),
	 ...
	 */
	//@Override
	public List<SearchResult<TItem, TDistance>> rangeSearch(TVector destination, TDistance ths)
	{
		...
		PriorityQueue<NodeIdAndDistance<TDistance>> topCandidates = rangeSearchBaseLayer(currObj, destination, ef, 0,
				ths);

		while (lt(topCandidates.peek().distance, ths))
		{
			ef = ef + 1;
			topCandidates = rangeSearchBaseLayer(currObj, destination, ef, 0, ths);
		}

		while (!topCandidates.isEmpty() && gt(topCandidates.peek().distance, ths))
		{
			topCandidates.poll();
		}
	...
	public List<SearchResult<TItem, TDistance>> rangeSearch2(TVector destination, TDistance ths)
	{
		...
		PriorityQueue<NodeIdAndDistance<TDistance>> topCandidates = rangeSearchBaseLayer2(currObj, destination, ef, 0,
				ths);
	...
	private PriorityQueue<NodeIdAndDistance<TDistance>> rangeSearchBaseLayer(Node<TItem> entryPointNode,
				...
				synchronized (node)
				{
					...
					for (int i = 0; i < candidates.size(); i++)
					{
						...
						if (!visitedBitSet.contains(candidateId))
						{
							...
							if (topCandidates.size() < efs)
							{
								candidateDistance = distanceFunction.distance(destination,
									candidateNode.item.vector());
							}
							else
							{
								candidateDistance = distanceFunction.distanceL2(destination,
										candidateNode.item.vector(),lowerBound);
							}
							
							
							if (topCandidates.size() < efs || gt(lowerBound, candidateDistance))
							//if (topCandidates.size() < efs || lt(candidateDistance,lowerBound))
							{
	...
	private PriorityQueue<NodeIdAndDistance<TDistance>> rangeSearchBaseLayer2(Node<TItem> entryPointNode,
			TVector destination, int efs, int layer, TDistance ths)
	{
		...
		try
		{
			...
			PriorityQueue<NodeIdAndDistance<TDistance>> rangeSet = new PriorityQueue<>();
			...
			while (!candidateSet.isEmpty())
			{
				...
				if (gt(currentPair.distance, lowerBound))
				{
					// if the distance of the topCandidate in the head position (that is, which has the greatest distance between the topCandidates)
					// is smaller than the threshold, it means that there may be more candidates smaller than the threshold, so the search must continue,
					// otherwise it can stop because it has already reached the local minimum.
					if (!(!topCandidates.isEmpty() && lt(topCandidates.peek().distance, ths)))
					{
						break;
					}
				...
				synchronized (node)
				{
					...
					for (int i = 0; i < candidates.size(); i++)
					{
						...
						if (!visitedBitSet.contains(candidateId))
						{
							...
							//if (gt(lowerBound, candidateDistance) || lt(candidateDistance,ths))
							//if (topCandidates.size() < efs || gt(lowerBound, candidateDistance))
							if (topCandidates.size() < efs || gt(lowerBound, candidateDistance) || lt(candidateDistance, ths))
							{
								...
								if (!candidateNode.deleted)
								{
									...
									// add into rangeSet
									if (lt(candidateDistance, ths))
										rangeSet.add(candidatePair);
								}

								// do not remove from the priority queue if the distance is less than the threshold.
								if (topCandidates.size() > efs && gt(topCandidates.peek().distance, ths))
								{
									topCandidates.poll();
								}
	...
	/**
	 * Creates a read only view on top of this index that uses pairwise comparision
	 * when doing distance search. And as such can be used as a baseline for
	 * assessing the precision of the index. Searches will be really slow but give
	 * the correct result every time.
	 *
	 * @return read only view on top of this index that uses pairwise comparision
	 *         when doing distance search
	 */
	...
	/**
	 * Returns the number of bi-directional links created for every new element
	 * during construction.
	 *
	 * @return the number of bi-directional links created for every new element
	 *         during construction
	 */
	public int getM()
	{
	...
	/**
	 * Set the size of the dynamic list for the nearest neighbors (used during the
	 * search)
	 *
	...
	/**
	 * Returns the parameter has the same meaning as ef, but controls the index time
	 * / index precision.
	 *
	 * @return the parameter has the same meaning as ef, but controls the index time
	 *         / index precision
	 */
	 ...
	 */
	public static <TId, TVector, TItem extends Item<TId, TVector>, TDistance> HnswIndex<TId, TVector, TItem, TDistance> load(
	...
	private static <TItem> Node<TItem> readNode(ObjectInputStream ois, ObjectSerializer<TItem> itemSerializer,
	...
	private static <TItem> AtomicReferenceArray<Node<TItem>> readNodesArray(ObjectInputStream ois,
			ObjectSerializer<TItem> itemSerializer, int maxM0, int maxM) throws IOException, ClassNotFoundException
	{
	...
	private static <TId> MutableObjectIntMap<TId> readMutableObjectIntMap(ObjectInputStream ois,
			ObjectSerializer<TId> itemIdSerializer) throws IOException, ClassNotFoundException
	{
	 ...
	 * @param <TVector>        Type of the vector to perform distance calculation on
	 * @param <TDistance>      Type of distance between items (expect any numeric
	 ...
	 * Start the process of building a new HNSW index.
	 *
	 * @param dimensions         the dimensionality of the vectors stored in the
	 *                           index
	 * @param distanceFunction   the distance function
	 * @param distanceComparator used to compare distances
	 * @param maxItemCount       maximum number of items the index can hold
	 * @param <TVector>          Type of the vector to perform distance calculation
	 *                           on
	 * @param <TDistance>        Type of distance between items (expect any numeric
	 *                           type: float, double, int, ..)
	 * @return a builder
	 */
	public static <TVector, TDistance> Builder<TVector, TDistance> newBuilder(int dimensions,
			DistanceFunction<TVector, TDistance> distanceFunction, Comparator<TDistance> distanceComparator,
			int maxItemCount)
	{
	...
	class ExactView implements Index<TId, TVector, TItem, TDistance>
	{
		...
		public List<SearchResult<TItem, TDistance>> findNearest(TVector vector, int k)
		{
			...
			Comparator<SearchResult<TItem, TDistance>> comparator = Comparator
					.<SearchResult<TItem, TDistance>>naturalOrder().reversed();
			...
			{ // if you iterate over a priority queue the order is not guaranteed
	...
	public static abstract class BuilderBase<TBuilder extends BuilderBase<TBuilder, TVector, TDistance>, TVector, TDistance>
	{
		...
		/**
		 * Sets the number of bi-directional links created for every new element during
		 * construction. Reasonable range for m is 2-100. Higher m work better on
		 * datasets with high intrinsic dimensionality and/or high recall, while low m
		 * work better for datasets with low intrinsic dimensionality and/or low
		 * recalls. The parameter also determines the algorithm's memory consumption. As
		 * an example for d = 4 random vectors optimal m for search is somewhere around
		 * 6, while for high dimensional datasets (word embeddings, good face
		 * descriptors), higher M are required (e.g. m = 48, 64) for optimal performance
		 * at high recall. The range mM = 12-48 is ok for the most of the use cases.
		 * When m is changed one has to update the other parameters. Nonetheless, ef and
		 * efConstruction parameters can be roughly estimated by assuming that m
		 * efConstruction is a constant.
		 *
		 * @param m the number of bi-directional links created for every new element
		 *          during construction
		 * @return the builder.
		 */
		...
		/**
		 * ` The parameter has the same meaning as ef, but controls the index time /
		 * index precision. Bigger efConstruction leads to longer construction, but
		 * better index quality. At some point, increasing efConstruction does not
		 * improve the quality of the index. One way to check if the selection of
		 * ef_construction was ok is to measure a recall for M nearest neighbor search
		 * when ef = efConstruction: if the recall is lower than 0.9, then there is room
		 * for improvement.
		 *
		...
		/**
		 * The size of the dynamic list for the nearest neighbors (used during the
		 * search). Higher ef leads to more accurate but slower search. The value ef of
		 * can be anything between k and the size of the dataset.
		 *
		...
		/**
		 * Call to enable support for the experimental remove operation. Indices that
		 * support removes will consume more memory.
		 *
		 * @return the builder
		 */
	...
	public static class Builder<TVector, TDistance> extends BuilderBase<Builder<TVector, TDistance>, TVector, TDistance>
	{
		...
		/**
		 * Build the index that uses java object serializers to store the items when
	...
	/**
	 * Extension of {@link Builder} that has knows what type of item is going to be
	 * stored in the index.
	 *
	 * @param <TId>       Type of the external identifier of an item
	 * @param <TVector>   Type of the vector to perform distance calculation on
			...
			extends BuilderBase<RefinedBuilder<TId, TVector, TItem, TDistance>, TVector, TDistance>
	{
		...
		RefinedBuilder(int dimensions, DistanceFunction<TVector, TDistance> distanceFunction,
				Comparator<TDistance> distanceComparator, int maxItemCount, int m, int ef, int efConstruction,
				boolean removeEnabled, ObjectSerializer<TId> itemIdSerializer, ObjectSerializer<TItem> itemSerializer)
		{
